...
- Host port `3200` available for the bundled git proxy (override with `GIT_PROXY_PORT`)
...
app/frontend release it ships** (e.g. `5.17.0`), and must match the
...
- Host port `3200` free for the bundled git proxy (override with
  `GIT_PROXY_PORT` in `/etc/openhexa/env.conf`)
...
Since OpenHEXA 5.11.0, users can clone, pull, and push static webapp
repositories directly with git. The package ships a `git_proxy` service
(image `nginx:alpine`, config at `/usr/share/openhexa/git-proxy/`) that
authenticates git clients against OpenHEXA OAuth and forwards allowed
traffic to Forgejo as a non-admin service account.

The relevant configuration properties:

- `GIT_PROXY_USERNAME` (default `openhexa-proxy`)
- `GIT_PROXY_PASSWORD`: auto-generated by `setup.sh` on first install; the
  Forgejo account is kept in sync by `openhexa.sh prepare`
- `GIT_PROXY_PORT` (default `3200`): host port mapped to the git proxy
- `GIT_PUBLIC_URL` (default `http://localhost:3200`): the public URL shown
  to users as the repository clone URL — point it at this host's
  `GIT_PROXY_PORT`, or at your reverse proxy if you put one in front
